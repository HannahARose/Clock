...
public:
  ...
  explicit XXHash64(uint64_t seed)
  {
    ...
    state[3] = seed - Prime1;
    bufferSize = 0;
    for (size_t i = 0; i < MaxBufferSize; i++) buffer[i] = 0;
    totalLength = 0;
  }
  ...
  uint64_t hash() const
  {
    ...
    for (; data + 8 <= stop; data += 8)
      result =
        rotateLeft(
          result ^ processSingle(0, *reinterpret_cast<const uint64_t *>(data)),
          27)
          * Prime1
        + Prime4;
    ...
    if (data + 4 <= stop) {
      result =
        rotateLeft(
          result ^ (*reinterpret_cast<const uint32_t *>(data)) * Prime1, 23)
          * Prime2
        + Prime3;
      data += 4;
    }
...
private:
  ...
  static inline void process(const void *data,
    ...
    uint64_t &state3)
  {
    const uint64_t *block = reinterpret_cast<const uint64_t *>(data);
